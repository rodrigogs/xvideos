...
const projectFiles = async (): Promise<string[]> => {
	const files = ROOT_FILES.filter((file) =>
	...
	for (const sourceDir of SOURCE_DIRS) {
		...
		files.push(
			...nestedFiles.map((file) => path.relative(process.cwd(), file)),
		);
	}
...
const actionableDiagnostics = (diagnostics: unknown[]): unknown[] => {
